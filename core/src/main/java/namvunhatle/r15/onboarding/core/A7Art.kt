 ...
 *
 * Every [Baked] is square, centred on its element's centre, [dp] wide in the design frame.
 */
class A7Art(ctx: Context) {
    ...
    private fun glow(rDp: Float, sigmaDp: Float, color: Int): Baked {
        ...
        return Baked(blur(disc(half, rDp, color), sigmaDp * PX), half * 2)
    }
    ...
    private fun layered(dDp: Float, shadows: List<Triple<Float, Float, Int>>, fill: Int): Baked {
        ...
        for ((r, sigma, color) in shadows.reversed()) {
            val g = blur(disc(half, r, color), sigma * PX)
    ...
    companion object {
        ...
        /** Gaussian blur ≈ 3 box blurs (premultiplied ARGB, so edges do not darken). */
        fun blur(src: Bitmap, sigma: Float): Bitmap {
            val w = src.width; val h = src.height
            val px = IntArray(w * h); src.getPixels(px, 0, w, 0, 0, w, h)
            val ch = Array(4) { FloatArray(w * h) }
            for (i in px.indices) {
                val c = px[i]; val a = (c ushr 24) / 255f
                ch[0][i] = a; ch[1][i] = ((c shr 16) and 255) / 255f * a; ch[2][i] = ((c shr 8) and 255) / 255f * a; ch[3][i] = (c and 255) / 255f * a
            }
            ...
            for (i in px.indices) {
                ...
                px[i] = ((a * 255f).roundToInt() shl 24) or (u(ch[1][i]) shl 16) or (u(ch[2][i]) shl 8) or u(ch[3][i])
            }
