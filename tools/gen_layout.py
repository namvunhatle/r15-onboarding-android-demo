#!/usr/bin/env python3
"""Generate app-views/src/main/res/layout/activity_main.xml from app-views/src/main/assets/manifest.json.
...
M = json.load(open(os.path.join(ROOT, 'app-views/src/main/assets/manifest.json')))
...
for d in ['paywall', 'ai', 'home']:
    ...
    dests.append(full(f'dest_{d}', I2, f'{I4}<ImageView android:layout_width="match_parent" android:layout_height="match_parent" android:scaleType="fitXY" android:importantForAccessibility="no" />{hot}'))
...
<!-- GENERATED by tools/gen_layout.py from the Figma export (manifest.json). Edit the script, not this file.
     ...
     android:tag = element id animated by the timeline in package …onboarding.core. -->
...
{sprite('statusbar', I2)}
        ...
        <!-- Destinations (static screenshots, set in code off the main thread, + hotspots) -->
{chr(10).join(dests)}
    </namvunhatle.r15.onboarding.views.DesignFrame>
